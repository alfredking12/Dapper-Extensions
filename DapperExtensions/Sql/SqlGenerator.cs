...
namespace DapperExtensions.Sql
{
    public interface ISqlGenerator
    {
        ...
        string Update<T>(IClassMapper classMap, T entity, IPredicate predicate, IDictionary<string, object> parameters) where T : DapperEntity;
    ...
    public class SqlGeneratorImpl : ISqlGenerator
    {
        ...
        public virtual string Update<T>(IClassMapper classMap, T entity, IPredicate predicate, IDictionary<string, object> parameters)
            where T : DapperEntity
        {
            if (predicate == null)
            {
            ...
            var columns = classMap.Properties.Where(p => !(p.Ignored || p.IsReadOnly || p.KeyType == KeyType.Identity || p.KeyType == KeyType.Assigned) && entity.IsUpdated(p.Name));
            if (!columns.Any())
            {
                throw new ArgumentException("No columns were mapped.");
            }
